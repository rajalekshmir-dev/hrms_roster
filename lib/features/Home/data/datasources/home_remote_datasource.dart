...
class HomeRemoteDataSourceImpl implements HomeRemoteDataSource {
  ...
  Future<List<DirectoryContactModel>> getEmployeeDirectory() async {
    try {
      final response = await authenticatedClient.get('/dashboard/employee_directory');
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load employee directory: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<List<DepartmentModel>> getDepartmentStats() async {
    try {
      final response = await authenticatedClient.get('/dashboard/department_stats');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData is List) {
          ...
          if (responseData.containsKey('departments') && responseData['departments'] is List) {
